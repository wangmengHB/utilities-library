...
Unlike other event emitter library, the Event Emitter in vscode has some special design:    
* NO Event Name Concept, each emitter represent one event name      
* Any exception in listener will not block the execuation of other listeners  
* Event is delivered in Order, especially in the scenarior of firing event in listener       

* PauseableEmitter (support pause/resume, and merge reducer)    
* AsyncEmitter
...
## 2. This Event Emitter will not block other handlers' execuation if any of them throw exception. For example: 
```ts
...
3. Event is delivered in order. If any event fire take place in a listener, it will push the event in the current delivery queue, and flush it immediately. For example:  
```ts
...
console.log(values); // ['e1', 'e2']
```
* Notice: To fire event in any listener should be aware of dead loop, condition statement is MUST.    
...
## AsyncEmitter
AsyncEmmiter is extended from EventEmitter, but with `async` method `fireAsync`. It requires:
1. Every event (it means the data fired by emitter) must implement IWaitUntil interface. (`waitUntil` method).   
2. `fireAsync` only accepts function as parameter like: `thenables => ({ waitUntil: t => thenables.push(t), ...otherData })`
For example: 
```ts
let globalState = 0;
